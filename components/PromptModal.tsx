...
import { X, Sparkles, Languages, Loader2, Plus, Variable, Target, Zap, FolderOpen, Wand2, Image as ImageIcon, UploadCloud, Link as LinkIcon, Command, Play, Terminal, Eye, History, Clock, ArrowRight, Save, RotateCcw, Copy } from 'lucide-react';
import { translateText, polishPrompt, magicFillPrompt, generatePromptFromImage, runPromptTest, generateVariations } from '../services/geminiService';
...
const PromptModal: React.FC<PromptModalProps> = ({ isOpen, onClose, onSave, initialData, collections, onAddCollection, addNotification, isReadOnly = false }) => {
  ...
  useEffect(() => {
    ...
    const detectedVars = matches.map(m => m[1]);
  ...
  const handleAIAction = async (action: 'translate' | 'polish' | 'magicFill' | 'imageToPrompt' | 'variations') => {
    ...
    try {
      if (action === 'translate') {
        ...
        addNotification('error', 'AI 操作失败，请检查 API Key。');
    } finally {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
              ...
              <div className="flex flex-col bg-slate-900 text-slate-200 p-4 rounded-xl border border-slate-800 shadow-inner">
                  <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-800">
                       ...
                       <div className="text-xs text-slate-500 font-mono">model: gemini-2.5-flash</div>
                  </div>
