...
const PromptCard: React.FC<PromptCardProps> = ({ 
  prompt, onEdit, onDelete, onTagClick, 
  isZenMode = false,
  selected = false,
  onSelect,
  selectionMode = false
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-800">
          <div className="flex flex-wrap gap-1 max-h-16 overflow-hidden">
            {prompt.techTags.map(t => <TagBadge key={t} label={t} type="tech" onClick={isZenMode ? undefined : (e) => { e?.stopPropagation(); onTagClick(t); }} />)}
            {prompt.styleTags.map(t => <TagBadge key={t} label={t} type="style" onClick={isZenMode ? undefined : (e) => { e?.stopPropagation(); onTagClick(t); }} />)}
            {prompt.customTags.map(t => <TagBadge key={t} label={t} type="custom" onClick={isZenMode ? undefined : (e) => { e?.stopPropagation(); onTagClick(t); }} />)}
          </div>
        </div>
