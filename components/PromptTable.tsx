                ...
                <td className="px-4 py-3 align-top">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {prompt.techTags.slice(0, 1).map(t => <TagBadge key={t} label={t} type="tech" onClick={() => onTagClick(t)} />)}
                      {prompt.styleTags.slice(0, 1).map(t => <TagBadge key={t} label={t} type="style" onClick={() => onTagClick(t)} />)}
                      {prompt.customTags?.slice(0, 2).map(t => <TagBadge key={t} label={t} type="custom" onClick={() => onTagClick(t)} />)}
                      {(prompt.techTags.length + prompt.styleTags.length + (prompt.customTags?.length || 0)) > 4 && (
                        <span className="text-[10px] text-slate-400 px-1">+{prompt.techTags.length + prompt.styleTags.length + (prompt.customTags?.length || 0) - 4}</span>
                      )}
                    </div>
                  </div>
                </td>
