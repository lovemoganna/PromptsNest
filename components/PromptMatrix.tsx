    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-x-auto">
      <div className="min-w-[800px] p-6">
        ...
        <div className="grid" style={{ gridTemplateColumns: `150px repeat(${scenes.length}, minmax(120px, 1fr))` }}>
          {/* Header Row: Scenes */}
          <div className="p-3 font-bold text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wider text-right border-b border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80">
            风格 \ 场景
          </div>
          {scenes.map(scene => (
            <div key={scene} className="p-3 font-semibold text-slate-700 dark:text-slate-300 text-xs border-b border-slate-200 dark:border-slate-800 text-center bg-slate-50 dark:bg-slate-800/50">
              {scene}
            </div>
          ))}
          ...
          {topStyles.map(style => (
            <React.Fragment key={style}>
              {/* Row Header */}
              <div className="p-3 font-medium text-slate-600 dark:text-slate-300 text-xs border-r border-slate-200 dark:border-slate-800 border-b dark:border-slate-800 flex items-center justify-end bg-slate-50 dark:bg-slate-800/50">
                {style}
              </div>
              ...
              {scenes.map(scene => {
                ...
                if (count > 0) bgClass = 'bg-blue-50 dark:bg-blue-900/10';
                if (count > 2) bgClass = 'bg-blue-100 dark:bg-blue-900/30';
                if (count > 5) bgClass = 'bg-blue-200 dark:bg-blue-900/50';

                return (
                  <div key={`${style}-${scene}`} className={`p-2 border-b border-slate-100 dark:border-slate-800 border-r border-slate-100 dark:border-slate-800 ${bgClass} transition-colors hover:bg-blue-50 dark:hover:bg-blue-900/30`}>
                    {count > 0 ? (
                      <div className="flex flex-col gap-1.5 h-full min-h-[60px]">
                        {cellPrompts.slice(0, 2).map(p => (
                          <div
                            key={p.id}
                            onClick={() => onEdit(p.id)}
                            className="bg-white dark:bg-slate-700 border border-blue-200 dark:border-slate-600 rounded px-2 py-1 text-[10px] text-slate-700 dark:text-slate-200 shadow-sm cursor-pointer hover:border-blue-400 dark:hover:border-blue-400 hover:shadow truncate"
                            title={p.title}
                          >
                            {p.title}
                          </div>
